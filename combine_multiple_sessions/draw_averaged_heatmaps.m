...
% input_path='S:\_Lea\2.Analysis_PhotoM_Behavior_IHC\PhotoM_Analysis\all_GCaMP_aIC_pIC\20250408_all_aIC\EPM_Zscore\output2\good_heatmaps\SD';
% output_path='S:\_Lea\2.Analysis_PhotoM_Behavior_IHC\PhotoM_Analysis\all_GCaMP_aIC_pIC\20250408_all_aIC\EPM_Zscore\output2\good_heatmaps\SD';
% batchID='test_EPM';

input_path='S:\_Lea\2.Analysis_PhotoM_Behavior_IHC\PhotoM_Analysis\all_aIC_aIC\OFT\heatmaps\HFD';
output_path='S:\_Lea\2.Analysis_PhotoM_Behavior_IHC\PhotoM_Analysis\all_aIC_aIC\OFT\heatmaps\HFD';
batchID='test_OFT';


[map, occ_map, experiment] = average_maps(input_path);

caxis_ = [-1, 1]
gaussian_sigma = []
customize_plot = 1

filename = fullfile(output_path,'Heatmap_wo_smoothing');
plot_heatmap(map, occ_map, filename, experiment, gaussian_sigma, caxis_, customize_plot, batchID);


gaussian_sigma = 0.75
filename = fullfile(output_path,'Heatmap_gaussian_smoothing');
plot_heatmap(map, occ_map, filename, experiment, gaussian_sigma, caxis_, customize_plot, batchID);
...
function [map, occ_map, experiment] = average_maps(input_path)

    dirOutput=dir(fullfile(input_path,'*.mat'));
    ...
    for i=1:n
        ...
        tmp_path=[input_path filesep tmp_file];
        load(tmp_path)
        if i==1
...
function plot_heatmap(map, occ_map, filename, experiment, gaussian_sigma, caxis_, customize_plot, batchID)
...
try
    ...
    map_mask = ones(size(map));
    warning('map_mask not implemented')
end
...
if strcmp(batchID, 'test_OFT') || strcmp(batchID, 'test_NSFT')
